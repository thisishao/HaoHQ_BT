<?php session_start();
	...
	$check = 1;
	$error_name = $error_email = $error_pwd  = $dk = "";
	if (isset($_POST['sub-acc'])) {
		if (empty($_POST['name'])) {
		...
		if ($check == 1) {
			$pwd = md5($_POST['pwd']);
			$sql = "UPDATE `user` SET `email`='".$_POST['email']."',`pass`='".$pwd."',`name`='".$_POST['name']."',`avatar`='".$_POST['name']."' WHERE `id` = ".$_GET['id']."";
			if ($result = $con->query($sql)) {
          		echo "<span style='color:red;'>Chỉnh sửa thông tin thành công Click vào <a href='index.php'>đây</a> để về trang chủ</span>";
          	}else{
   				...
   				<div class="col-sm-9 padding-right">
   					<form action="" method="POST">
	   					<div class="row sm-3">
						    ...
						    <div class="col-sm-6">
						      <input type="text" class="form-control" name="name" value="<?php echo $data['name'] ?>"><span style='color:red;'><?php echo $error_name ?></span>
						    </div>
						</div>
						<div class="row sm-3">
						...
						<button type="submit" class="btn btn-primary" name="sub-acc">Update</button>
					</form>
   				</div>
	    	</div>  
    	</div>	
